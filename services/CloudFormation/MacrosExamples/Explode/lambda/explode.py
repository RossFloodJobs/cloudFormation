...
def replace_explode_in_string(value, map_data):
    ...
    while match:
        ...
        if isinstance(replace_value, int):
            value = replace_value
            # No furr explosion is possible on an int
            match = None
        else:
...
def handle_section_transform(section, mappings):
    """Go through template and explode objects in  section."""
    new_section = {}
...
def handle_transform(fragment):
    """Go through template and explode objects in  fragment."""
...
def handler(event, _context):
    """Handle invocation in Lambda (when CloudFormation processes  Macro)"""
...
if __name__ == "__main__":
    """
    If run from  command line, parse  file specified and output it
    This is quite naive; CF YAML tags like !GetAtt will break it (as will
    !Explode, but you can hide in a string). Probably best to use JSON.
